...
describe.each(['request', 'reply', 'event'])('%s channel events',
   ...
   */
  (type) => {
    it.each(['flow', 'drain', 'recover', 'discard', 'retry', 'pause', 'resume', 'lost', 'remove'])('should re-emit %s',
      /**
...
it.each(['open', 'close', 'error', 'reconnect', 'exhausted'])('should re-emit %s from connection',
  /**
   * @param {comq.diagnostics.Event} event
   */
  async (event) => {
